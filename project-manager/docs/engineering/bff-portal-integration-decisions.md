...
| **既存の `NEXT_PUBLIC_PROFILE_API_ENDPOINT`** | BFF 経由に寄せるなら、将来的に **`/api/portal/me` のような BFF パス**に変更し、クライアントは同一オリジンのみを見る。 |
...
| **PHP `dashboard.php` との揃え** | 現状の `dashboard.php` のピルは **portal と同じ `portalFetchAppsForUser` の `effective_role`（全案件最大）**。**案件管理 Web** を **案件単位 permission 正**にすると、**PHP 側も同じ基準に変えるか**、ラベルを分けるかを決める。 |
...
5. 余力で `GET /me` も BFF 経由に寄せ、`NEXT_PUBLIC_PROFILE_API_ENDPOINT` を段階的に廃止。
...
| [platform-common/router.php](../../../platform-common/router.php) | `/portal/api/apps` ルーティング。 |
| [project-manager/apps/web/app/components/DashboardShell.tsx](../../apps/web/app/components/DashboardShell.tsx) | プロフィール取得などクライアント `fetch` あり。BFF 化時に差し替え候補。 |
